...
export default class BaiTapLayoutMap extends Component {
    ...
    renderLayoutPhim = () => {
        const contenJSX = this.dataPhim.map((product, index) => {
            return <div className="card-deck col-4 mt-5">
                <div className="card">
                    <div style={{ background: `url('${product.hinhAnh}') center`, backgroundSize: "cover" }}>
                        <img style={{ opacity: "0" }} height={300} className="card-img-top" src={product.hinhAnh} alt={product.tenPhim} />
                    </div>
